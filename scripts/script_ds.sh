...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate -o orderer.ptunstad.no:7050 -C $CHANNEL_NAME -n myccds -v 1.2 -c '{"Args":[""]}' -P "OR('Org1MSP.member','Org1MSP.member')" >&log.txt
	else
		peer chaincode instantiate -o orderer.ptunstad.no:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n myccds -v 1.2 -c '{"Args":[""]}' -P "OR('Org1MSP.member','Org1MSP.member')" >&log.txt
	fi
	res=$?
	cat log.txt
...
chaincodeQuery () {
  ...
  do
     ...
     peer chaincode query -C $CHANNEL_NAME -n myccds -c '{"Args":["queryState","TV"]}' >&log.txt
...
chaincodeInvoke () {
	...
	# lets supply it directly as we know it using the "-o" option
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode invoke -o orderer.ptunstad.no:7050 -C $CHANNEL_NAME -n myccds -c '{"Args":["addIoT","HomeCam","0", "0", "0"]}' >&log.txt
	else
		peer chaincode invoke -o orderer.ptunstad.no:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n myccds -c '{"Args":["addIoT","HomeCam","0", "0", "0"]}'
	fi
	res=$?
	cat log.txt
